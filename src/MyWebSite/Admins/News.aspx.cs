    ...
    public partial class News : System.Web.UI.Page
    {
        ...
        private void View()
        {
            if (Session["IsAdmin"] != null)
            {
                ...
                {
                    // Hiển thị toàn bộ tin mặc định cho tài khoản quản trị
                    grdNews.DataSource = NewsService.News_GetByTop("", "", "Date desc, Id desc");
                    grdNews.DataBind();
                    if (grdNews.PageCount <= 1)
                    {
                        grdNews.PagerStyle.Visible = false;
        ...
        {
            // Bypass date mask issues: only validate required fields; we parse/normalize date ourselves
            if (rfvGroupNews.IsValid && rfvName.IsValid)
            {
                ...
                // Set date automatically on update; on insert, parse user's input with fallback to now
                ...
                else
                {
                    ...
                    DateTime parsedDate;
                    // Try Vietnamese 24h then 12h, then en-US as last resort
                    string[] formats = new[] { "dd/MM/yyyy HH:mm:ss", "dd/MM/yyyy hh:mm:ss tt", "MM/dd/yyyy HH:mm:ss", "MM/dd/yyyy hh:mm:ss tt" };
                    if (!DateTime.TryParseExact(raw, formats, new CultureInfo("vi-VN"), DateTimeStyles.None, out parsedDate))
                    {
                        if (!DateTime.TryParseExact(raw, formats, new CultureInfo("en-US"), DateTimeStyles.None, out parsedDate))
                        {
                ...
                obj.Date = DateTimeClass.ConvertDateTime(effectiveDate, "yyyy-MM-dd HH:mm:ss");
